...
Два необязательных SDK, ни один не вложен в репозиторий (I11). Для локальной
сборки их скачивают вручную; релизный workflow добывает их сам — DLSS SDK из
публичного репозитория NVIDIA на закреплённом коммите, заголовки Optical Flow
из секрета репозитория (см. *Как выпустить релиз* ниже). DLL среды выполнения —
другое дело: это не SDK, сборке они не нужны, и в релизном архиве они уже лежат.
...
Тег и есть релиз. Задайте `VERSION` в `CMakeLists.txt`, напишите раздел в
`CHANGELOG.md` и запушьте `vX.Y.Z` на `main`; остальное делает
`.github/workflows/release.yml` и публикует результат в **Releases**. Ему нужны
оба SDK, а Optical Flow SDK не публичный, поэтому репозиторий должен передать
его два заголовка секретом — около 15 КБ, без какого-либо хостинга:
...
Четыре сторонних файла берутся из архива, из которого был собран релиз 0.1.2, и
сверяются с дайджестами, закреплёнными в workflow; переменная репозитория
`THIRD_PARTY_BUNDLE_URL` перенаправит задание, если тот архив когда-нибудь
исчезнет. Если не сходится дайджест, версия или раздел журнала, задание
отказывается публиковать, а не отгружает что-то молча другое.
